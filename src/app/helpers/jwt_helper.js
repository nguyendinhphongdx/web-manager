...
module.exports = {
    ...
    tokenProducts: (req, res, next) => {
        
        const keyProduct = req.headers.authorization.split(' ')[1];
        if (!keyProduct) return next(creatError.Unauthorized());
        req.keyProduct = keyProduct;
        next();
    }
}
